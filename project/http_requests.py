...
def putMember_Class(property,value,member_id,class_id,token,host):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to get Members name. Status code: {response.status_code}, Response: {response.text}")
        return False
def _update_class_attendance(percents,member_id,class_id,token,host):
    #type: (float,int,int,str,str) -> bool   
    isPresent=False
    if percents >=0.5: isPresent=True
    status1=putMember_Class("setPresent",isPresent,member_id,class_id,token,host)
    status2=putMember_Class("setAttendance_percents",float(percents),member_id,class_id,token,host)
